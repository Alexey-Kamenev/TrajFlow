...
def transform_preds_to_waymo_format(pred_dicts, top_k_for_eval=-1, eval_second=8, inter_pred=False):
    ...
    for scene_idx, val in enumerate(scene2preds.items()):
        ...
        for obj_idx, cur_pred in enumerate(preds_per_scene):
            ...
            if not inter_pred:
                ...
                batch_pred_trajs[scene_idx, :, :, obj_idx] = cur_pred['pred_trajs'][np.newaxis, :topK, 4::sampled_interval, :][:, :, :num_frame_to_eval, :]
                batch_pred_scores[scene_idx, 0] = cur_pred['pred_scores'][:topK]
            ...
            if not inter_pred:
                ...
                pred_gt_idxs[scene_idx, :, obj_idx] = obj_idx
                pred_gt_idx_valid_mask[scene_idx, :,  obj_idx] = 1
...
def waymo_evaluation(pred_dicts, top_k=-1, eval_second=8, num_modes_for_eval=6, inter_pred=False):

    pred_score, pred_trajectory, gt_infos, object_type_cnt_dict = transform_preds_to_waymo_format(
        pred_dicts, top_k_for_eval=top_k, eval_second=eval_second, inter_pred=inter_pred
    )
    ...
    gt_is_valid = tf.convert_to_tensor(gt_infos['gt_is_valid'], tf.bool)
    pred_gt_indices = tf.convert_to_tensor(gt_infos['pred_gt_indices'], tf.int64)
    pred_gt_indices_mask = tf.convert_to_tensor(gt_infos['pred_gt_indices_mask'], tf.bool)
